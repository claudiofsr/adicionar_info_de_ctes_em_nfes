...
impl Config {
    ...
    pub fn append<'a>(&self, field: &mut Cow<'a, str>, value: &str, label: &str) {
        ...
        let sufixo = if label == "NF-e" {
            format!(" [Info da {}: {}]", label, value)
        } else {
            format!(" [Info do {}: {}]", label, value)
        };

        // Verificação Unicode-aware para não cortar caracteres no meio
        let tamanho_atual = field.chars().count();
        let tamanho_sufixo = sufixo.chars().count();
